...
class MgRL_CE_Net(nn.Module):
    ...
    def __init__(
            self, granularity_dict: Dict[str, int], ga_K: int,
            encoding_input_size: int, encoding_hidden_size: int, negative_sample_num: int,
            device: torch.device
    ):
...
if __name__ == '__main__':
    ...
    model = MgRL_CE_Net(granularity_dict=g_dict, ga_K=2, encoding_input_size=2, encoding_hidden_size=16, negative_sample_num=5, device=dev)
    out = model(mg_input)
    print(out["pred"].shape)
    print(out["contrastive_loss"].shape)
