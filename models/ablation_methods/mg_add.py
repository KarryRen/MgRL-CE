...
class Mg_Add_Net(nn.Module):
    ...
    def __init__(
        ...
        """ The init function of Mg_Add Net.
        ...
        super(Mg_Add_Net, self).__init__()
        self.device = device
        ...
        # Granularity 1 (coarsest)
        self.feature_encoder_dict["g1"] = NoRec_FeatureEncoder(input_size=encoding_input_size, hidden_size=encoding_hidden_size).to(device=device)
        # Granularity 2 (fine)
        self.feature_encoder_dict["g2"] = NoRec_FeatureEncoder(input_size=encoding_input_size, hidden_size=encoding_hidden_size).to(device=device)
        # Granularity 3 (finer)
        self.feature_encoder_dict["g3"] = NoRec_FeatureEncoder(input_size=encoding_input_size, hidden_size=encoding_hidden_size).to(device=device)
        # Granularity 4 (finer)
        self.feature_encoder_dict["g4"] = NoRec_FeatureEncoder(input_size=encoding_input_size, hidden_size=encoding_hidden_size).to(device=device)
        # Granularity 5 (finest)
        self.feature_encoder_dict["g5"] = NoRec_FeatureEncoder(input_size=encoding_input_size, hidden_size=encoding_hidden_size).to(device=device)

    def forward(self, mul_granularity_input: Dict[str, torch.Tensor]) -> dict:
        """ The forward function of Mg_Add Net.
...
if __name__ == "__main__":
    ...
    model = Mg_Add_Net(granularity_dict=g_dict, ga_K=1, encoding_input_size=1, encoding_hidden_size=7, device=dev)
    out = model(mg_input)
